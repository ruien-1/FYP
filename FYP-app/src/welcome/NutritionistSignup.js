...
import { 
  signupNutritionist, 
  checkNutritionistEmailVerified 
} from "../nutritionistauth";
import API from "../api/backend";
import * as FileSystemLegacy from "expo-file-system/legacy";
import { SafeAreaView } from 'react-native-safe-area-context';

const NutritionistSignup = ({ navigation, onBack }) => {
  ...
  const handleNutritionistSignUp = async () => {
    ...
    try {
      const result = await signupNutritionist(email, password);
      if (!result.success) {
  ...
  const handleContinueAfterVerification = async () => {
    ...
    try {
      const verified = await checkNutritionistEmailVerified(email, password);
      ...
      Alert.alert(
        "Success!",
        "Your nutritionist account has been created! Your account will be reviewed and activated shortly.",
      ...
      >
          ...
          <View style={styles.inner}>
            ...
            <View style={styles.infoBox}>
              ...
              <Text style={styles.infoText}>
                Accepted documents: Professional license, certification, or diploma (PDF, PNG, JPG)
              </Text>
            </View>
            ...
            <View style={styles.documentTypeContainer}>
              {[
                { value: "license", label: "Professional License", desc: "State/National license to practice" },
                { value: "certification", label: "Certification", desc: "RD, RDN, CNS, or other credentials" },
                { value: "diploma", label: "Degree/Diploma", desc: "Nutrition or Dietetics degree" },
        ...
        {step === 9 && (
          <View style={styles.inner}>
            ...
            <View style={styles.summaryContainer}>
              <Text style={styles.summaryTitle}>Practice Details</Text>
              <Text style={styles.summaryText}>Location: {profile.placeOfPractice}</Text>
              {profile.clinicName && (
                <Text style={styles.summaryText}>Clinic: {profile.clinicName}</Text>
              )}
              <Text style={styles.summaryText}>
                Virtual Consultations: {profile.offersVirtualConsultation}
              </Text>
            </View>
            ...
            <View style={styles.summaryContainer}>
              ...
              <Text style={styles.summaryText}>Document Type: {
                ...
                profile.documentType === 'certification' ? 'Certification' :
            ...
            {step === 10 && (
              <View style={styles.inner}>
                ...
                <TouchableOpacity
                  ...
                  onPress={handleNutritionistSignUp}
                >
