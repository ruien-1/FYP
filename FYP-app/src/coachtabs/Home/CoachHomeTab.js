...
export default function CoachHomeTab() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [
        ...
        pendingWorkoutPlansRes,
        confirmedWorkoutPlansRes,
        articlesRes,
      ] = await Promise.all([
      ...
      const pendingWorkoutPlans = pendingWorkoutPlansRes.data?.data || [];
      const confirmedWorkoutPlans = confirmedWorkoutPlansRes.data?.data || [];
      const fetchedArticles = articlesRes.data?.data || [];

      setPendingRequests(pendingAppointments.length + pendingWorkoutPlans.length);
      setPendingActions(confirmedWorkoutPlans.length);
  ...
  const handlePendingRequestPress = () => {
    navigation.navigate("CoachPendingRequest");
  };

  const handlePendingActionPress = () => {
    navigation.navigate("CoachPendingAction");
  };

  const handleAppointmentPress = (appointment) => {
    navigation.navigate("CoachUpcomingAppointment", {
  ...
  const handleAddArticle = () => {
    navigation.navigate("CoachArticleManagement");
  };
  ...
  const isValidUrl = (string) => {
    if (!string) return true; // Empty is valid (optional)
    try {
      new URL(string);
      return true;
    } catch (_) {
  ...
  const handleUpdateArticle = async () => {
    ...
    if (editArticleLink.trim() && !isValidUrl(editArticleLink.trim())) {
    ...
    try {
      ...
      const updateData = {
        ...
        articleLink: editArticleLink.trim() || null,
        photos: allPhotos,
      };
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              const response = await API.delete(`/coacharticle/${articleId}`);
        ...
        >
          <View style={styles.modalContainer}>
            ...
            <ScrollView style={styles.modalContent} contentContainerStyle={{ paddingBottom: 30 }}>
              ...
              {/* Update Button */}
              <TouchableOpacity
                style={[styles.updateButton, uploading && styles.updateButtonDisabled]}
