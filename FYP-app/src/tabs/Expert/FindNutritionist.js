...
export default function FindNutritionist() {
  ...
  const toggleExpandCard = (nutritionistId) => {
    setExpandedCards(prev => ({
      ...prev,
      [nutritionistId]: !prev[nutritionistId]
    }));
  };
  ...
  const applyFilters = (search, specializations, services, experience, dataSource = null) => {
    ...
    if (search.trim() !== "") {
      filtered = filtered.filter(
        (nutritionist) =>
          ...
          nutritionist.services?.some((serv) =>
    ...
    if (specializations.length > 0) {
      filtered = filtered.filter((nutritionist) =>
        specializations.some(spec => 
          nutritionist.specializations?.includes(spec)
        )
      );
    }
    ...
    if (services.length > 0) {
      filtered = filtered.filter((nutritionist) =>
        services.some(service => 
          nutritionist.services?.includes(service)
        )
      );
    }
    ...
    if (experience) {
      ...
      filtered = filtered.filter(
        (nutritionist) => parseInt(nutritionist.yearsOfExperience) >= expYears
      );
    }
  ...
  const renderSpecializations = (specializations, nutritionistId) => {
    if (!specializations || specializations.length === 0) {
      return <Text style={styles.nutritionistSpecialty}>Nutrition Expert</Text>;
    }

    const isExpanded = expandedCards[nutritionistId];
    const displayedSpecs = isExpanded ? specializations : specializations.slice(0, 3);
    const hasMore = specializations.length > 3;

    return (
      <View>
        <Text style={styles.nutritionistSpecialty} numberOfLines={isExpanded ? undefined : 2}>
          {displayedSpecs.join(", ")}
          {!isExpanded && hasMore && "..."}
        </Text>
        {hasMore && (
          <TouchableOpacity onPress={() => toggleExpandCard(nutritionistId)}>
        ...
        {filteredNutritionists.length === 0 ? (
          ...
          <>
            <Text style={styles.resultsText}>
              {filteredNutritionists.length} nutritionist
              {filteredNutritionists.length !== 1 ? "s" : ""} found
            </Text>
            {filteredNutritionists.map((nutritionist) => (
              <View key={nutritionist.id} style={styles.nutritionistCard}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    navigation.navigate("NutritionistProfile", {
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Filter Nutritionists</Text>
